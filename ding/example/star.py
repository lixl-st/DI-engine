...
from ding.framework import Parallel, task
from ding.framework.context import BattleContext
from ding.framework.middleware import OffPolicyLearner, data_pusher, eps_greedy_handler, nstep_reward_enhancer
from ding.utils import set_pkg_seed
from ding.utils.sparse_logging import log_every_n, log_every_sec
...
def coordinator_mocker(cfg):
    ...
    def _coordinator_mocker(ctx):
        log_every_sec(logging.INFO, 60, "[Mock Coordinator]: alive")
        time.sleep(1.0)
    ...
    def _actor_mocker(ctx):
        ...
        for _ in range(6):
            for player in players:
                log_every_n(logging.INFO, 13, "[Mock Actor]: actor player: {}".format(player.player_id))
